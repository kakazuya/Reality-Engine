...
class ReportWriter:
    ...
    def write_json(self, report: DailyAlphaReport, path: Path) -> Path:
        ...
        Gracefully includes Wave D4 ensemble metadata (MoE weights + per-thesis lens
        activation) when the orchestrator attached it as an instance attribute (pydantic
        models reject arbitrary fields, so it is merged in post-hoc here, never required).
        """
        path.parent.mkdir(parents=True, exist_ok=True)
        data = report.to_dict() if hasattr(report, "to_dict") else report.dict()

        # Enrich with the ensemble synthesis metadata attached by the orchestrator.
        ems = getattr(report, "ensemble_metadata", None)
        if ems is not None:
            data["ensemble_metadata"] = ems

        # Per-thesis MoE activation (attached via object.__setattr__ by the orchestrator).
        theses_objs = getattr(report, "high_conviction_theses", []) or []
        tlist = data.get("high_conviction_theses", []) or []
        for i, t in enumerate(theses_objs):
            act = getattr(t, "ensemble_activation", None)
            if act is not None and i < len(tlist):
                tlist[i]["ensemble_activation"] = act
    ...
    def write_markdown(self, report: DailyAlphaReport, path: Path) -> Path:
        ...
        # Wave D4: Ensemble (MoE) synthesis footer — emitted only when present.
        ems = getattr(report, "ensemble_metadata", None)
        if ems is not None:
            md_lines.extend([
                "## 6. Ensemble (Sparse MoE) Synthesis Metadata",
                "",
                f"- **Synthesis Mode:** `{ems.get('mode', 'ensemble')}`",
                f"- **Investor Majority Cohort:** `{ems.get('investor_majority', 'all')}`",
                f"- **MoE Temperature (explore/exploit):** `{ems.get('temperature')}`",
                f"- **Candidates Evaluated:** `{ems.get('n_candidates')}`",
                f"- **Avg Lenses Fired:** `{ems.get('avg_n_fired')}` (avg fired-weight-sum `{ems.get('avg_fired_weight_sum')}`)",
            ])
            w = ems.get("ensemble_weights")
            if isinstance(w, dict):
                md_lines.append("")
                md_lines.append("**Per-lens-family ensemble weights (Σ = 1.0):**")
                for fam, wt in w.items():
                    md_lines.append(f"  - `{fam}`: {float(wt):.4f}")
            tg = ems.get("transient_graphs")
            if tg:
                md_lines.append("")
                md_lines.append("**Transient event-graph context (recent spawned ripples):**")
                for g in tg:
                    md_lines.append(
                        f"  - `{g.get('event_id')}`: biggest beneficiary `{g.get('biggest_beneficiary')}` (S={g.get('S')})"
                    )
            md_lines.extend(["", "---", ""])
    ...
    def write_html(self, report: DailyAlphaReport, path: Path) -> Path:
        ...
        # Wave D4: Ensemble (MoE) synthesis section — emitted only when metadata present.
        ems = getattr(report, "ensemble_metadata", None)
        ensemble_section_html = ""
        if ems is not None:
            w = ems.get("ensemble_weights") or {}
            weight_badges = " ".join(
                f"<span class=\"pill pill-mod\">{html.escape(str(k).split('_')[0])}: {float(v):.3f}</span>"
                for k, v in w.items()
            ) if w else "<span class=\"text-muted\">N/A</span>"
            tg = ems.get("transient_graphs") or []
            tg_rows = "".join(
                f"<li><span class=\"bullet-icon bullet-green\">◆</span> {html.escape(str(g.get('event_id')))} → "
                f"beneficiary <strong>{html.escape(str(g.get('biggest_beneficiary')))}</strong> (S={g.get('S')})</li>"
                for g in tg
            )
            ensemble_section_html = f"""
        <div class="section-header">🧠 Ensemble (Sparse MoE) Synthesis Metadata</div>
        <div class="table-container">
            <table>
                <thead>
                    <tr>
                        <th>Mode</th>
                        <th>Investor Majority</th>
                        <th>Temperature</th>
                        <th>Candidates</th>
                        <th>Avg Lenses Fired</th>
                        <th>Fired-Weight-Sum</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td><strong>{html.escape(str(ems.get('mode', 'ensemble')))}</strong></td>
                        <td>{html.escape(str(ems.get('investor_majority', 'all')))}</td>
                        <td class="text-center font-mono">{ems.get('temperature')}</td>
                        <td class="text-center">{ems.get('n_candidates')}</td>
                        <td class="text-center">{ems.get('avg_n_fired')}</td>
                        <td class="text-center font-mono">{ems.get('avg_fired_weight_sum')}</td>
                    </tr>
                </tbody>
            </table>
        </div>
        <div class="thesis-body-grid">
            <div class="thesis-section-block">
                <h4 class="section-subtitle"><span class="icon">⚖</span> Per-Lens-Family Ensemble Weights (Σ = 1.0)</h4>
                <p class="section-text">{weight_badges}</p>
            </div>
            <div class="thesis-section-block">
                <h4 class="section-subtitle"><span class="icon">🌊</span> Transient Event-Graph Context</h4>
                <ul class="cat-risk-list">
                    {tg_rows if tg_rows else '<li class="text-muted">No recent transient graphs spawned.</li>'}
                </ul>
            </div>
        </div>
            """
...
<body>
    <div class="container">
        ...
        <!-- Wave D4: Ensemble (MoE) synthesis section -->
        {ensemble_section_html}
